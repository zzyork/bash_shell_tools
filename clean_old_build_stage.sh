...
function clean_old_build_stage(){
    ...
    for i in ${ALL_APP[@]};do
        # echo -e "当前项目：${i%?}"
        appnum=`ls ${WORKDIR} | grep -Eo ${i}\([0-9].*?\) | wc -l`
        time=`ls -tl ${WORKDIR} | grep -E ${i}\([0-9].*?\) | awk '{print $8}' | uniq | wc -l`
        echo -e "\n不同创建时间的项目数：${appnum}\n"
        if [ ${appnum} -gt 5 ]; then
            if [ ${time} -gt 3 ]; then
            del_appnum=`expr ${appnum} - 5`
            # echo -e "待删除数量：${del_appnum}\n"
            del_applist=`ls -tr ${WORKDIR} | grep -Eo ${i}\([0-9].*?\) | head -${del_appnum}`
